...
/** Strips HTML tags and decodes entities to plain text for structured rendering */
function stripHtml(html: string): string {
    ...
    // First pass: strip real tags
    let text = stripTags(html);
    // Decode entities — this may expose tags that were entity-encoded inside a wrapper
    text = text
        .replace(/&amp;/g, "&")
        .replace(/&lt;/g, "<")
        .replace(/&gt;/g, ">")
        ...
        .replace(/&quot;/g, '"')
        .replace(/&#39;/g, "'");
    // Second pass: strip any tags revealed by entity decoding
    text = stripTags(text);
    return text.replace(/\n{3,}/g, "\n\n").trim();
}
...
export default function AboutPage() {
    ...
    const content = rawContent.trimStart().startsWith("<") ? stripHtml(rawContent) : rawContent;
        ...
        <div className="min-h-screen bg-white">
            ...
            >
                {isLoading ? (
                    ...
                    /* Render structured sections — HTML is stripped to plain text first */
                    <div className={`space-y-2 text-[15px] leading-[1.9] text-black/75 font-medium ${isRtl ? "text-right" : "text-left"}`}>
